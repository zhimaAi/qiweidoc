...
func startRR(name string, rpcPort, httpPort int, needPlugins []string) (*endure.Endure, error) {
    ...
    if name == "main" {
        overrides = append(overrides, fmt.Sprintf(`http.pool.num_workers=2`))
    }
    ...
    if name == "main" {
        overrides = append(overrides, fmt.Sprintf(`cron.pool.num_workers=4`))
    }
    ...
    if name == "main" {
        overrides = append(overrides, fmt.Sprintf(`micro.pool.num_workers=2`))
    }
