	...
	methods: {
		...
		setCheck(item, index, type) {
			if (type === 1) {
				...
				let flag = this.currSelectKeys.findIndex(i => i === item.department_id)
				if (flag !== -1) {
					this.currSelectKeys.splice(flag, 1)
					this.setTreeKeys()
				}
      ...
      //单选
			if (this.selectType !== 'multiple') {
				this.currSelectKeys = [department_id]
        this.setTreeKeys()
			} else {
				//多选
        if (this.currSelectKeys.includes(department_id)) {
          ...
          this.currSelectKeys.push(department_id)
          this.setTreeKeys()
        }
      }
			// this.setCheckData()
		...
		checkChange(item, status) {
			...
			if (is_staff) {
				...
				if (status && !ids.includes(item.department_id)) {
				...
				} else if (!status && ids.includes(item.department_id)) {
					let flag
		...
		//设置树形选中
		setTreeKeys() {
			this.$refs.elTreeRef.setCheckedKeys(this.currSelectKeys)
		},
		//获取组织奖架构列表
		getFrameWorkList() {
			this.spinning = true
            ...
            // return setTimeout(() => {
              ...
              // this.isSession || this.excludeRole || this.group_chat || this.bind_type  去掉这四个条件
							i.disabled = i.staff_count * 1 > 500 || this.selectType !== 'multiple'
							ids.push(i.department_id)
		...
		async loadMoreTap(node, data) {
			...
			this.staff_list_params.department_id = data.par_department_id
			...
			res.forEach((i, index) => {
				...
				i.department_id = data.par_department_id + "_" + i.staff_id
	...
	// 		})
      ...
      if(this.viewAppointPermissions){
        params.permission_staff_ids = this.viewAppointPermissions
			...
			return this.$api.user_get_staff_list(params).then(res => {
				return Promise.resolve(res.data.list)
			}).catch(err => {
				return Promise.resolve([])
			})
