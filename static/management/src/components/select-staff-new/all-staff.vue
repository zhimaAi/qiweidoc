...
<template>
	<div class="all-staff">
		<a-input-search
			placeholder="请输入员工昵称或员工别名进行搜索"
			@search="addPeopleSearch"
			style="margin-top: -8px;"
			v-model.trim="staff_list_params.keyword"
		/>
		<div class="old-select" v-if="oldSelect.length">
			...
			<div class="items flex">
				<div
					...
					v-for="(item, index) in oldSelect"
					:key="index"
					@click="oldSelectTap(item, index)"
				>
					<img class="img" :src="item.avatar" alt="">
					<!-- <div class="name">{{item.name}}</div> -->
					<div
						class="name-wra fx-ac"
						:class="{'oth-name-wra': oldSelect.length === 2 || oldSelect.length === 4}"
					>
						<a-tooltip>
							<template #title>
								<WWOpenData
		...
		<div
			class="datas-item"
			:style="{height: oldSelect.length ?  oldSelect.length > 3 ?'312px' : '348px' : '410px'}"
			v-infinite-scroll="peopleListLoad"
      infinite-scroll-distance="20"
		>
			<div
				class="all-memeber fx-ac"
				v-for="(item, index) in staff_list"
				:key="index"
				@click="wraperTap(item,index)"
			>
				...
				<div class="i-left fx-ac">
					<div @click.stop="">
						<el-checkbox
							...
							@change="checkChange(item,$event, index)"
						></el-checkbox>
					</div>
					<img
						:src="item.avatar"
				...
				<div class="i-right flex">
					<div class="ri-top fx-ac">
						...
						<div class="nums fx-ac">
							...
							<div style="color: rgba(0, 0,0 ,.65); font-weight: 500;">{{item.external_total}}</div>
						</div>
						<!-- <div class="c-j flex" v-if="item.role == 1">超级管理员</div>
						<div class="p-t flex" v-if="item.role == 2">普通员工</div> -->
					</div>
					<div class="ri-bot fx-ac">
						<!-- <div style="color: rgba(0, 0,0 .45)">客户数：</div>
						<div style="color: rgba(0, 0,0 .65)">{{item.external_total}}</div> -->
						<!-- 别名： -->
  ...
  watch: {
      ...
      for (let i=0;i < this.staff_list.length;i++) {
        ...
        this.$set(this.staff_list, i, staff)
			...
			for (let index = 0; index < this.oldSelect.length; index ++) {
				...
				this.$set(this.oldSelect, index, staff)
			}
    },
    ...
    getUser_get_staff_list() {
      ...
      // 仅显示没有席位的
			...
			if (this.sortType) {
				this.staff_list_params.sort_by_cst_num = this.sortType == 1 ? 'desc' : 'asc'
			} else {
        this.$delete(this.staff_list_params,'sort_by_cst_num')
			}
      // 指定员工的员工
      if(this.viewAppointPermissions){
        this.staff_list_params.permission_staff_ids = this.viewAppointPermissions
      }
      this.$api.user_get_simple_staff_list(this.staff_list_params).then(res => {
    ...
    checkChange(item, e, ind) {
      ...
      this.getUser_get_staff_list()
        ...
        //     this.oldSelect = this.oldSelect.filter(sub => this.viewAppointPermissions.includes(sub.staff_id))
        //   }
		// 		}
		// 	})
		},
		oldSelectTap(item, index) {
			...
			this.$set(this.oldSelect, index, item)

			let dataList = JSON.parse(JSON.stringify(this.selectData))
			let ids = dataList.filter(i => i.data_type === 1).map(i => i.staff_id)

			if (!ids.includes(item.staff_id)) {
				item.data_type = 1
				//单选
				if (this.selectType !== 'multiple') {
					dataList= [item]
		...
		sessionCheck(e) {
			...
			// console.log(this.local_bind_type);
			this.staff_list = []
      this.staff_list_nomore = false
      this.staff_list_params.page  = 1
      this.getUser_get_staff_list()
