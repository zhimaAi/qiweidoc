<!--
 * @Author: Mango
 * @Date: 2022-08-08 09:54:47
 * @LastEditors: Mango
 * @LastEditTime: 2022-09-08 21:09:47
-->
<template>
	<div class="framework">
		<a-input-search
			placeholder="请输入组织架构进行搜索"
			:disabled="search_Loading"
			v-model.trim="searchParams.keyword"
		/>
		<div class="title flex">
			<div v-if="!hasSelect" class="fir">请选择一个组织架构</div>
			<div class="bread flex" v-else>
				<div class="left flex" @click="goBackOne">
					<img src="../../assets/image/select-staff-new/bacnk.png" alt="">
					<div>返回</div>
				</div>
				<div class="right-tit eliOne fx-ac">
					<!-- {{selectTitle}} -->
					<WWOpenData style="display:inline-block" type='departmentName' :showStatus="showStatus" :openid='staff_list_params.department_id'>
						<span class="wk-black-65-text">{{ selectTitle }}</span>
					</WWOpenData>
				</div>
			</div>
		</div>
		<div class="big-wraper">
			<a-spin :spinning="textures_spinning" v-if="!hasSelect">
				<div class="textures-wraper" v-if="group_list.length">
					...
					>
						<div class="left flex">
							<img src="../../assets/image/select-staff-new/zu-zhi.png" alt="">
							<div class="name fx-ac">
								<!-- {{item.department_name}} -->
								<WWOpenData style="display:inline-block" type='departmentName' :showStatus="showStatus" :openid='item.department_id'>
									<span class="wk-black-65-text">{{ item.department_name }}</span>
								</WWOpenData>
							</div>
						</div>
						<div class="right flex" @click.stop="">
							<el-checkbox
								...
								size="small"
								@change="checkChange(item,$event, 2)"
				...
				<a-empty description="暂无搜索组织架构" v-if="!group_list.length" />
			</a-spin>
			<div class="one-texture" v-else>
				...
				>
					<div class="textures flex"
						v-for="(item, index) in child_textures"
	...
	methods: {
		...
		setCheck(item, index, type) {
			if (type === 1) {
				...
				if (this.child_textures.length) {
					let ids = this.child_textures.map(i => i.department_id)
					let flag = ids.indexOf(item.department_id)
					if (this.child_textures[flag]) {
						this.child_textures[flag].checked = false
						this.$set(this.child_textures, flag, this.child_textures[flag])
					}
				}

				if (this.group_list.length) {
					let ids = this.group_list.map(i => i.department_id)
					let flag = ids.indexOf(item.department_id)
				...
				if (this.textures_list.length) {
					let ids = this.textures_list.map(i => i.department_id)
					let flag = ids.indexOf(item.department_id)
		...
		selectPush(item, index) {
			...
			this.selectTitle = item.department_name
			this.selectItems.push(item)
			this.child_textures = item.child

			this.staff_list_nomore = false
			this.staff_list_params.department_id = item.department_id
		...
		onSearch() {
			...
			if (!this.searchParams.keyword) {
				...
				const getName = e => {
					e.forEach(i => {
						if (i.department_name.indexOf(this.searchParams.keyword) !== -1) {
							arr.push(i)
      ...
      this.$api.user_get_staff_list(this.staff_list_params).then(res => {
		...
		checkChange(item, e, type, ind) {
			...
			if (type === 1) {
				...
				let ids = dataList.filter(i => i.data_type === 2).map(i => i.department_id)
				if (e && !ids.includes(item.department_id)) {
				...
				} else if (!e && ids.includes(item.department_id)) {

					let flag
					dataList.forEach((i, ind) => {
						if (i.department_id === item.department_id) {
							flag = ind
						}
					})
