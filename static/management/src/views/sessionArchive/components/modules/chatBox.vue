<template>
  <div class="chat-box">
    <div class="header">
      <div class="title">
        ...
        <template v-else-if="chatInfo?.sender && chatInfo?.receiver">
          <ChatCollection v-if="!(chatInfo?.receiver?.zm_user_type === 'STAFF' && chatInfo?.params?.tab === 'LOAD_BY_STAFF')" @changeCollect="changeCollectFn" :currentMsgCancelCollect="currentMsgCancelCollect"
            :chatInfo="chatInfo" />
    ...
    <ZmScroll :key="msgCompKey" :loading="loading" :finished="finished" :immediate="false"
      :showLoading="list.length > 0" ref="listRef" loadType="up" @load="loadData"
      :class="['message-list', { opacity: opacity }]">
      <!-- , {'message-list-tips': chatInfo?.params?.is_collect && chatInfo?.params?.collect_reason} -->
      <a-empty v-if="finished && list.length < 1" description="暂无数据" style="margin-top: 60px;" />
      <template v-for="(item, index) in list">
        <div v-if="item.msg_type != 'revoke'" :key="item.msg_id" :class="['message-item', { isSelf: item.isSelf }]">
          <div class="user-info">
